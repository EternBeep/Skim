...
export default function App() {
  ...
  // Once a video is indexed, pull its beat/energy analysis.
  useEffect(() => {
    if (!videoId) return;
    setAudio(null);
  ...
  async function handleIndex() {
    ...
    try {
      const res = await fetch(`${API}/index`, {
        ...
        body: JSON.stringify({ youtube_url: url, interval_sec: 2.0 }),
      });
      const data = await res.json();
  ...
  const previewVid = extractVideoId(url);
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", padding: "64px 24px 40px" }}>
        ...
        }}>Week 2 build · CLIP + Whisper + librosa + ChromaDB</div>
        <h1 style={{
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 20px" }}>
              ...
              {results.map((r, i) => (
                <a key={i} href={r.youtube_url} target="_blank" rel="noreferrer"
                  ...
                  <div style={{
                    ...
                    {r.thumbnail_url
                      ? <img src={`${API}${r.thumbnail_url}`} alt=""
        ...
        <div style={{
          ...
          <div style={{ marginTop: 20, padding: "12px 0 0", borderTop: "1px solid #2a2a38", fontSize: 11, color: "#444450" }}>
            Vectors persist in ChromaDB · librosa surfaces the beat grid &amp; energy peaks for music videos
          </div>
        </div>
